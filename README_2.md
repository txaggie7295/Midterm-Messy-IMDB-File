...
Cleaning was performed in Excel using **Data → Get Data → From Text/CSV** for the import (with semicolon delimiter and UTF-8 encoding) and a combination of formulas, Find & Replace, and manual edits for value cleanup. Each column was repaired individually with the original retained side-by-side until verified.
...
**Release Year**
- Source contained at least seven different date formats: `1995-02-10`, `09 21 1972`, `22 Feb 04`, `23rd December of 1966`, `10-29-99`, `01/16-03`, and others. Standardized to a single consistent format.

**Duration**
- Replaced placeholder values (`Nan`, `Inf`, `NULL`, `Not Applicable`, `-`, blank) with empty cells.
- Stripped stray characters from numeric values (e.g., `178c` → `178`).

**Country**
- Standardized variants: `US` → `USA`, `New Zesland` → `New Zealand`, `New Zeland` → `New Zealand`.

**Income**
- Stripped currency symbols and whitespace.
...
**Votes**
- Source used European-style dot separators (`2.278.845`). Converted to plain integers.

**Score**
- The noisiest column. Issues included trailing decimals (`9.`), comma decimals (`9,.0`), embedded letters (`8,9f`), leading zeros (`08.9`), double decimals (`8..8`), wrong delimiter (`8:8`), leading symbols (`++8.7`), trailing decimals (`8.7.`), and scientific notation fragments (`8,7e-0`). Each row repaired to a clean decimal between 0 and 10.

**Genre and Director**
- Multi-value fields split into separate columns (Genre1, Genre2, Genre3; Director1, Director2) to support pivot analysis.
...
- `IMDB_Analysis.xlsx` — cleaned dataset, pivot tables, and density calculations
