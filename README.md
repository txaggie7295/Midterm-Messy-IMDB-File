...
SELECT 
    score_tier,
    COUNT(*) AS films,
    ROUND(100.0 * COUNT(*) / (SELECT COUNT(*) FROM movies), 1) AS pct_of_films,
    ROUND(AVG(income), 0) AS avg_income,
    ROUND(100.0 * SUM(income) / (SELECT SUM(income) FROM movies), 1) AS pct_of_income,
    ROUND(
        (100.0 * SUM(income) / (SELECT SUM(income) FROM movies)) /
        (100.0 * COUNT(*) / (SELECT COUNT(*) FROM movies)),
        2
    ) AS density
FROM tiered
GROUP BY score_tier
